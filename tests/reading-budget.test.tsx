 ...
 *
 * 量的是 ReportPage 自己渲染的內容，不含站台外框（導覽、頁尾）——
 * 外框每頁都一樣，不是這一頁的閱讀負擔。收起的 <details> 內容會被扣掉，
 * 所以這個數字代表「使用者一進來真的要面對多少字」。改版前是 4,257 字。
 *
 * 2026-07-30 從 800 調到 840：報告下方加了「把這張盤變成一張圖」的入口
 * （標題一行 + 說明一行，約 40 字）。宇宙印記原本只存在於實驗室目錄裡，
 * 使用者找不到，而它是這個站最適合被分享出去的產出之一。
 *
 * 這不是塞內容，是加一個入口。調整前我先把新寫的那兩行盡量縮短，
 * 確認五個樣本的最壞情況（1990-01-02）是 825 字、光靠縮短文案下不到 800，
 * 才調整上限；新上限＝實測最壞值 825 加 15 字餘裕，不是取一個好聽的整數。
 */
const OVERVIEW_VISIBLE_MAX = 840;
