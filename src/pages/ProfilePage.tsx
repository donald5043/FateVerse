...
export default function ProfilePage() {
  ...
  const updateCity = (value: string) => {
    setError('');
    const match = lookupCityCoordinates(value);
    setCityNotice(match ? `已自動帶入${match.city}的經緯度，可再手動微調。` : '');
    setForm((current) => ({
      ...current,
      city: value,
      ...(match ? { longitude: match.longitude, latitude: match.latitude } : {}),
    }));
  };
      ...
      <form onSubmit={submit} className="mt-7 grid items-start gap-7 lg:grid-cols-[1fr_0.72fr]" noValidate>
        <div className="glass-card space-y-6 p-5 sm:p-7">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <label><span className="label">出生城市（選填）</span><input className="input-field" list="city-options" value={form.city} onChange={(e) => updateCity(e.target.value)} placeholder="例：臺北市（會自動帶入經緯度）" /><datalist id="city-options">{CITY_SUGGESTIONS.map((city) => <option value={city} key={city} />)}</datalist>{cityNotice && <span className="mt-1.5 block text-xs text-emerald-200" role="status">{cityNotice}</span>}</label>
        ...
        <aside className="glass-card p-5 sm:p-7 lg:sticky lg:top-24">
          ...
          <button className="btn-primary mt-6 hidden w-full lg:inline-flex" type="submit" disabled={busy}>{busy ? '正在計算…' : '完成計算並查看報告'}<ArrowRight size={18} /></button>
        </aside>
