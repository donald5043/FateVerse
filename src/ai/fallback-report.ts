...
export function generateFallbackReport(input: FateReportInput): AiFateReport {
  ...
  return {
    ...
    sections: {
      ...
      ...(input.ziwei ? { ziwei: `紫微排盤為${input.ziwei.fiveElementsClass}，命主${input.ziwei.soul}、身主${input.ziwei.body}，命宮在${input.ziwei.soulPalaceBranch}、身宮在${input.ziwei.bodyPalaceBranch}。命宮主星欄為「${soulPalaceStars}」。星曜需連同十二宮、三方四正與流派設定閱讀，本版先呈現可驗證盤面，不直接由單星斷吉凶。` } : {}),
