...
- 西洋星盤：十個星體的地心黃道位置、太陽／月亮星座、逆行與主要相位；提供十星元素／模式分布、相位緊密度，並在有經緯度時加入上升、等宮制／整宮制十二宮、角續果宮與人生領域比較。
- 生命靈數：保留 11、22、33 大師數，顯示完整加總過程。
- 姓名分析：約 200 個常用姓名用字的字義與現代筆畫（可手動修正），並在筆畫齊全時計算五格剖象（天／人／地／外／總格）、81 數理吉凶（常見通行版本）與三才五行關係。
...
內建約 200 個常用姓名用字（`src/data/name-dictionary.ts`），含字義、簡化五行對照與現代標準筆畫；筆畫計法有流派差異的字（如艸部）刻意不提供筆畫。新增筆畫資料時必須標示來源類型；沒有完整康熙字典資料時不可標成正式康熙筆畫。
