...
- 姓名分析：750+ 個常用姓名用字的字義、簡化五行對照與現代筆畫（可手動修正），並在筆畫齊全時計算五格剖象（天／人／地／外／總格）、81 數理吉凶（常見通行版本）與三才五行關係。
...
內建 750+ 個常用姓名用字（`src/data/name-dictionary.ts`），含字義、簡化五行對照與現代標準筆畫；筆畫計法有流派差異的字（如艸部）刻意不提供筆畫。新增筆畫資料時必須標示來源類型；沒有完整康熙字典資料時不可標成正式康熙筆畫。
